...
#define XPR_BAD_INO	    2

#define TEST_BUFFER_BLOCKS	16
#define MAX_GOOD_BLOCKS		512

#define UPPER(size,n)	((size +(n)-1)/(n))
#define	INODE_SIZE	(sizeof(struct xpr_inode))

#define INODE_BLOCKS	UPPER(INODES,XPR_INODES_PER_BLOCK)
...
static int magic=XPR_MAGIC;

static char root_block[BLOCK_SIZE]="\0";

static char * inode_buffer=NULL;

#define Inode	(((struct xpr_inode *)inode_buffer)-1)

static char super_block_buffer[BLOCK_SIZE];
static char boot_block_buffer[512];

#define Super	(*(struct xpr_super_block *)super_block_buffer)	
#define INODES	((unsigned long)Super.s_ino_cnt)
#define ZONES	((unsigned long)Super.s_blk_cnt)
#define IMAPS ((unsigned long)s_imap_blocks)
#define ZMAPS ((unsigned long)s_zmap_blocks)


#define FIRSTZONE	((unsigned long)Super.s_fdblk)
#define BLOCKSIZE	((unsigned long)Super.s_log_bsize)
...
static void make_bad_inode(void){
	struct xpr_inode *inode=&Inode[XPR_BAD_INO];
	int i,j,zone;
	int ino=0;
	int block;
...
#define NEXT_BAD	(zone=next(zone))
	...
	inode->i_blk_start=block;
	inode->i_blk_end=block;
	inode->i_blocks=1;
	for(i=0;i<512;i++){
		bad_buffer[i]=zone;
		if(!NEXT_BAD)
...
static void make_root_inode(void){
	...
	struct xpr_inode *inode=&Inode[XPR_ROOT_INO];
	mark_inode(XPR_ROOT_INO);
	inode->i_blk_start=get_free_block();
	inode->i_blk_end=inode->i_blk_start;
	inode->i_blocks=1;
	inode->i_nlinks=2;
	//inode->i_time=time(NULL);
	if(badblocks)
	...
	if(inode->i_uid)
		inode->i_gid=getgid();
	write_block(inode->i_blk_start,root_block);
}
...
static void setup_tables(void){
	...
	Super.s_magic=magic;
	Super.s_log_bsize=0;
	Super.s_blk_cnt=BLOCKS;
	
	inodes=BLOCKS/3;
	inodes=((inodes+XPR_INODES_PER_BLOCK -1)& ~(XPR_INODES_PER_BLOCK -1));
	Super.s_ino_cnt=inodes;
	s_imap_blocks=UPPER(INODES+1,BITS_PER_BLOCK);
	s_zmap_blocks=UPPER(BLOCKS-(1+IMAPS+INODE_BLOCKS),BITS_PER_BLOCK+1);
	Super.s_fdblk=NORM_FIRSTZONE;
	Super.s_link_max=XPR_LINK_MAX;
	...
	printf("%ld Inodes\n",INODES);
	printf("%ld Blocks\n",ZONES);
	printf("First data block is %ld(%ld)\n",FIRSTZONE,NORM_FIRSTZONE);
	printf("Blocksize=%d\n",BLOCK_SIZE<<ZONESIZE);
}
...
int main(int argc,char **argv){
	char *tmp;
	struct stat statbuf;
	if(INODE_SIZE*XPR_INODES_PER_BLOCK!=BLOCK_SIZE)
		die("Bad inode size");
	magic=XPR_MAGIC;
	device_name=argv[1];
